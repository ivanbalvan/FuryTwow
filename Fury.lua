...
local function DoUpdateConfiguration(defaults)
    local configs = {
      ...
      {"SunderArmorStacks", 5},   -- Set how many Sunder Armor stacks to apply (1–5)
	  ...
	  {ABILITY_SLAM_FURY, true},               -- Use Slam as Arms filler
...
function Fury()
 FuryAOEStatus = Fury_Configuration[MODE_HEADER_AOE] and "enabled" or "disabled"
...
elseif FurySweepingStrikes
  ...
  and IsSpellReady(ABILITY_SWEEPING_STRIKES_FURY) then
    if GetActiveStance() ~= 1 then
        Debug("22. Battle Stance (Sweeping Strikes)")
        if not FuryOldStance then
            FuryOldStance = GetActiveStance()
        end
        DoShapeShift(1)
...
elseif Fury_Configuration[MODE_HEADER_PROT]
  ...
  and IsSpellReady(ABILITY_THUNDER_CLAP_FURY) then
    Debug("XX. Thunder Clap (Prot + AOE)")
    ...
    FuryLastSpellCast = GetTime()
        ...
        elseif FuryBloodthirst
          ...
          and IsSpellReady(ABILITY_BLOODTHIRST_FURY) then
            ...
            FuryLastSpellCast = GetTime()
-- Sunder Armor (apply 1–5 stacks based on config, 0 = disabled)
elseif tonumber(Fury_Configuration["SunderArmorStacks"] or 5) > 0
  and UnitMana("player") >= 10
  and IsSpellReady(ABILITY_SUNDER_ARMOR_FURY)
  and not HasDebuff("target", "Ability_Warrior_Sunder", tonumber(Fury_Configuration["SunderArmorStacks"])) then

    Debug("27. Sunder Armor (target below "..tostring(Fury_Configuration["SunderArmorStacks"])..")")
...
-- Slam (Arms filler, only if next swing is far away)
elseif Fury_Configuration[ABILITY_SLAM_FURY]
  and HasWeapon()
  and not Fury_Configuration[MODE_HEADER_AOE]
  and FuryMortalStrike
  and UnitHealth("target") / UnitHealthMax("target") > 0.25
  and not IsSpellReady(ABILITY_MORTAL_STRIKE_FURY)
  and UnitMana("player") >= 15
  and IsSpellReady("Slam")
  and (weaponSwingSpeed - (GetTime() - lastMeleeSwingTime)) > 2 then
    Debug("Slam (enough time before next swing)")
    ...
    FuryLastSpellCast = GetTime()
        ...
        elseif (Fury_Configuration[MODE_HEADER_AOE]
          ...
          or not FuryWhirlwind))  then
            ...
            -- Cleave (only if Whirlwind is on cooldown)
            elseif (Fury_Configuration[ABILITY_CLEAVE_FURY]
              or Fury_Configuration[MODE_HEADER_AOE])
              and HasWeapon()
              and not IsSpellReady(ABILITY_WHIRLWIND_FURY) then

                if Fury_Configuration[ABILITY_SWEEPING_STRIKES_FURY]
                  and HasBuff("player", "Ability_Rogue_SliceDice")
                  and GetActiveStance() ~= 3 then
                    Debug("53. Cleave (switch to Berserker Stance for SS + Cleave)")
                    if not FuryOldStance then
                        FuryOldStance = GetActiveStance()
                    end
                    DoShapeShift(3)
...
local function Fury_ScanTalents()
    ...
    -- Check for Flurry
    local _, _, _, _, currRank = GetTalentInfo(2, 14)
    if currRank > 0 then
        Debug("Flurry")
        FuryFlurry = true
    else
    ...
    local _, _, _, _, currRank = GetTalentInfo(3, 14)
    if currRank > 0 then
        Debug("Shield Slam")
        FuryShieldSlam =  true
    else
...
["sunder"] = { help = "Set number of Sunder Armor stacks to apply (1-5).", fn = function(options)
    SetOptionRange("SunderArmorStacks", "Sunder Armor Stacks", options, 0, 5)
end },
...
-- Track melee swings for Slam timing (fully 1.12-compatible)

local slamSwingFrame = CreateFrame("Frame", "SlamSwingFrame")
slamSwingFrame:RegisterEvent("CHAT_MSG_COMBAT_SELF_HITS")
slamSwingFrame:RegisterEvent("CHAT_MSG_SPELL_SELF_DAMAGE")

slamSwingFrame:SetScript("OnEvent", function()
    if event and arg1 then
        local msg = string.lower(arg1)
        if string.find(msg, "you hit") or 
           string.find(msg, "you crit") or 
           string.find(msg, "your heroic strike") then
           
            lastMeleeSwingTime = GetTime()
          --  DEFAULT_CHAT_FRAME:AddMessage("Swing Detected! Time: " .. lastMeleeSwingTime)
        end
    end
end)
